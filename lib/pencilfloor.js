...
class Pencilfloor extends HTMLElement
{
    constructor()
    {
        ...
        `
        :host { background: white; display: inline-flex; }
        :host([hidden]) { display: none; }
        canvas { flex: auto; min-height: 0; width: 100%; }
        span
        {
        ...
        span:focus { box-shadow: 0 0 2px 3px #3B99FC; outline: none; }
        svg
        {
        ...
        /*
        { visibility: visible; } makes the element actually visible for a short time in Blink,
        despite ancestors being invisible. Many other animatable properties don't work in all
        browsers.
...
(pencilfloor, initParams) =>
{
    ...
    {
        const { host } = base.getRootNode();
        if (host)
            host.dispatchEvent(new Event(evtType));
    }
    ...
    function drawStrokes()
    {
        for (const pencil of pencils)
        {
            ...
            if (abs(x - x0) < minDistance && /* c8 ignore next */ abs(y - y0) < minDistance)
            /* c8 ignore next 5 */
            {
                ctx.fillStyle = color;
    ...
    {
        shadowRoot.interactive = Boolean(value);
        updateFocusability();
    }
    ...
    const width = toSize(initParams.width, DEFAULT_WIDTH);
    const height = toSize(initParams.height, DEFAULT_HEIGHT);
    const pencilSize =
    (() =>
    {
        let pencilSize = Number(initParams.pencilSize);
        if (pencilSize <= 0)
            return 0;
    ...
    const pencils = [];
    {
        let pencilParams = initParams.pencils;
        ...
        pencilParams = Object(pencilParams);
    ...
    (
        pencilfloor,
        {
            ...
            interactive: defineAccessorProperty(() => shadowRoot.interactive, setInteractive),
    ...
    const { shadowRoot } = pencilfloor;
    const base = document.createElement('SPAN');
    {
        const oldBase = shadowRoot.querySelector('span');
        const fnName = oldBase ? 'replaceChild' : 'appendChild';
        shadowRoot[fnName](base, oldBase);
    }
    updateFocusability();
    base.tabIndex = -1;
    ...
    (
        ...
        evt =>
        {
            if (shadowRoot.interactive && evt.key === ' ')
            {
                togglePlay(true);
                evt.stopPropagation();
            }
    ...
    (
        ...
        evt =>
        {
            if (shadowRoot.interactive && evt.button === 0)
            {
                togglePlay(true);
                evt.stopPropagation();
            }
...
customElements.define('html-pencilfloor', Pencilfloor);
